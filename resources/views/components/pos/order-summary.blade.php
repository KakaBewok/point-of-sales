...
<aside {{ $attributes->merge(['class' => 'flex flex-col bg-white dark:bg-zinc-950 mt-6 md:mt-0 md:border-l border-t md:border-t-0 border-zinc-200 dark:border-zinc-800 shrink-0 w-full md:w-[40%] lg:w-[35%] xl:w-[30%] shadow-2xl overflow-visible md:overflow-hidden relative z-10']) }}>
    ...
    <div class="px-3 py-3 bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-100 dark:border-zinc-800 shrink-0 flex gap-2">
        @if($manualDiscountType && $manualDiscountValue > 0)
            <div class="flex-1 flex flex-col bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md overflow-hidden relative group">
                ...
                <button wire:click="resetDiscount" class="cursor-pointer absolute right-0 top-0 bottom-0 w-8 flex items-center justify-center text-amber-700 hover:text-amber-800 font-medium hover:bg-amber-100 dark:hover:bg-amber-900/50 transition-colors border-l border-amber-200 dark:border-amber-800/50">
                    <flux:icon name="x-mark" class="h-4 w-4" />
                </button>
            </div>
        @else
            <button wire:click="openDiscountModal" class="cursor-pointer flex-1 py-3 px-2 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 border-2 border-zinc-200 dark:border-zinc-700 rounded-md text-[10px] font-black uppercase text-zinc-600 dark:text-zinc-300 shadow-sm transition-colors flex items-center justify-center gap-1.5">
        ...
        @if($voucherApplied)
            <div class="flex-1 flex flex-col bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-md overflow-hidden relative group">
        ...
        @else
            <button wire:click="openVoucherModal" class="cursor-pointer flex-1 py-3 px-2 bg-zinc-900 dark:bg-zinc-700 hover:bg-zinc-800 dark:hover:bg-zinc-600 border-2 border-zinc-900 dark:border-zinc-700 rounded-md text-[10px] font-black uppercase text-white shadow-sm transition-colors flex items-center justify-center gap-1.5">
                <flux:icon name="ticket" class="h-5 w-5" />
                Voucher
            </button>
        @endif
    </div>
    ...
    <footer class="shrink-0 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-800 p-5 space-y-4 shadow-[0_-10px_20px_0_rgba(0,0,0,0.05)]">
        <div class="space-y-2">
            ...
            @if($totalDiscount > 0)
                <div class="flex justify-between text-[10px] font-bold text-green-500 uppercase tracking-widest leading-none">
                    <span>Diskon</span>
                    <span class="font-black text-green-500">- Rp{{ number_format($totalDiscount, 0, ',', '.') }}</span>
                </div>
            @endif
            @if($taxRate > 0)
                <div class="flex justify-between text-[10px] font-bold text-red-500 uppercase tracking-widest">
                    <span>Pajak ({{ $taxRate }}%)</span>
                    <span class="text-red-500 dark:text-zinc-300">+ Rp{{ number_format($taxAmount, 0, ',', '.') }}</span>
                </div>
            @endif
            
            <div class="flex justify-between items-end pt-2 border-t border-black/5 dark:border-zinc-800">
                <div class="flex flex-col gap-2">
                    <span class="text-[9px] uppercase tracking-[0.2em] mb-0.5 font-bold text-black">Total Tagihan</span>
                    <span class="text-3xl font-black text-zinc-950 dark:text-white tracking-wider leading-none">
        ...
        @if(count($cart) > 0)
            <button 
                ...
                class="cursor-pointer w-full h-14 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-black uppercase tracking-[0.2em] shadow-lg shadow-green-200 dark:shadow-none transition-all active:scale-95 group overflow-hidden relative"
            >
