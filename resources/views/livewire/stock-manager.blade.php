<div class="px-0.5 md:px-6 py-8 space-y-8 max-w-7xl mx-auto flex-1 w-full min-h-screen">
    <div class="flex items-center justify-between">
        <h1 class="text-xl md:text-2xl font-black tracking-normal text-zinc-900 dark:text-white">Manajemen Stok</h1>
    </div>
    ...
    <div class="flex flex-col gap-4 sm:flex-row">
        <div class="flex-1 w-full">
            <flux:input icon="magnifying-glass" class="h-10 text-sm" wire:model.live.debounce.300ms="search" placeholder="Cari nama produk atau SKU..." />
        </div>
        <div class="w-full sm:w-64">
            <flux:select wire:model.live="filterType" class="border-zinc-300 h-10 text-sm cursor-pointer" placeholder="Semua Produk">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        {{-- Product Stock Table --}}
        <div class="col-span-1 lg:col-span-2 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xl shadow-zinc-200/50 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none min-h-[400px] flex flex-col">
            <div class="overflow-x-auto flex-1 customized-scrollbar">
                <table class="w-full text-sm text-left whitespace-nowrap">
                    <thead class="bg-zinc-50 border-b border-zinc-200 dark:bg-zinc-950 dark:border-zinc-800">
                        <tr>
                            <th class="px-6 py-5 text-xs font-black uppercase tracking-widest text-zinc-400">Produk</th>
                        ...
                        @forelse($products as $product)
                            <tr class="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50 group">
                                <td class="px-4 py-3">
                                    ...
                                    <div class="text-[10px] font-black tracking-widest text-zinc-400 mt-1 uppercase">{{ $product->category->name }}</div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span class="text-sm font-black {{ $product->stock <= $product->low_stock_threshold ? 'text-red-500' : 'text-zinc-900 dark:text-white' }}">{{ $product->stock }}</span>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <button wire:click="openAdjustmentModal({{ $product->id }})" class="cursor-pointer inline-flex items-center justify-center h-10 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest bg-zinc-100 hover:bg-zinc-200 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 transition-all active:scale-95 border border-zinc-200 dark:border-zinc-700">
        ...
        {{-- Recent Logs Panel --}}
        <div class="col-span-1 rounded-xl border border-zinc-200 bg-white p-6 md:p-8 shadow-xl shadow-zinc-200/50 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none flex flex-col min-h-[400px]">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                           ...
                           <p class="text-[10px] font-black uppercase tracking-widest text-zinc-400 mt-1">
                                ...
                                @php
                                    $isIncrease = $log->stock_after > $log->stock_before;
                                    $isDecrease = $log->stock_after < $log->stock_before;
                                    $isSame = $log->stock_after === $log->stock_before;
                                @endphp

                                <span class="{{ $isIncrease ? 'text-emerald-500' : ($isDecrease ? 'text-red-500' : 'text-zinc-500') }}">
                                    @if($isIncrease)
                                        +{{ $log->quantity }}
                                    @elseif($isDecrease)
                                        -{{ $log->quantity }}
    ...
    {{-- Adjustment Modal --}}
    <flux:modal wire:model="showAdjustmentModal" class="max-w-sm md:max-w-md p-0 overflow-hidden bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl">
        @if($selectedProduct)
        <div>
            <header class="border-b border-zinc-100 dark:border-zinc-800 pb-4 mb-4">
                ...
                <p class="text-xs text-zinc-500 mt-1">Penyesuaian stok untuk produk yang dipilih.</p>
            </header>

            <div class="max-h-[60vh] overflow-y-auto pr-2 customized-scrollbar">
                <form wire:submit="processAdjustment" id="stockForm" class="p-3 space-y-4">
                    <div class="p-4 rounded-lg bg-zinc-50 border border-zinc-100 dark:bg-zinc-800/50 dark:border-zinc-700/50 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-sm font-semibold text-zinc-900 dark:text-white leading-tight">{{ Str::limit($selectedProduct->name, 10) }}</p>
                        </div>
                        <div class="text-right">
                            <span class="text-xs font-medium text-zinc-500 tracking-wider">Stok Saat Ini</span>
                            <div class="text-sm font-bold text-zinc-900 dark:text-white leading-none mt-1">{{ $selectedProduct->stock }}</div>
                        </div>
                    </div>
                    ...
                    <flux:field>
                        ...
                        <flux:textarea wire:model="adjustmentNotes" rows="3" placeholder="Keterangan..." class="mt-1 rounded-lg border-zinc-300 focus:border-green-500 focus:ring-green-500" />
            ...
            <footer class="mt-6 flex justify-end gap-3 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                <button type="button" class="cursor-pointer h-10 px-4 rounded-lg bg-zinc-100 text-zinc-700 hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 font-medium text-sm transition-colors" wire:click="$set('showAdjustmentModal', false)">Batal</button>
                <button type="submit" form="stockForm" class="cursor-pointer h-10 px-6 rounded-lg bg-green-600 hover:bg-green-700 text-white font-semibold text-sm transition-colors">Simpan</button>
            </footer>
        </div>
        @endif
    </flux:modal>
