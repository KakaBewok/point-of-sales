<div class="px-1 py-8 md:px-6 space-y-8 max-w-7xl mx-auto flex-1 w-full">
    <div class="flex items-center justify-between">
        <h1 class="text-lg md:text-xl font-bold tracking-tight text-zinc-900 dark:text-white">Riwayat Pergerakan Stok</h1>
    ...
    <div class="flex flex-col gap-4 sm:flex-row">
        <div class="flex-1">
            <flux:input icon="magnifying-glass" class="h-9 text-sm" wire:model.live.debounce.300ms="search" placeholder="Cari nama produk..." />
        </div>
        <div class="w-full sm:w-56">
            <flux:select wire:model.live="typeFilter" class="h-10 text-sm" placeholder="Semua Tipe">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                    @forelse($logs as $log)
                        <tr class="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                            <td class="px-6 py-4 text-zinc-600 dark:text-zinc-400 whitespace-nowrap text-sm">{{ $log->created_at->format('d/m/Y H:i') }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="text-sm px-6 py-4 font-medium text-zinc-900 dark:text-white">{{ $log->product->name ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-center font-semibold text-zinc-600 dark:text-zinc-300 text-sm">{{ $log->stock_after }}</td>
                            <td class="px-6 py-4 text-zinc-600 dark:text-zinc-400 text-sm">{{ $log->user->name ?? 'Sistem' }}</td>
