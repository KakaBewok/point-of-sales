<div class="px-6 py-8 md:px-8 space-y-8 max-w-7xl mx-auto flex-1 w-full">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Manajemen Pengguna</h1>
        <flux:button variant="primary" icon="plus" class="h-10 px-4" wire:click="create">Tambah Pengguna</flux:button>
    </div>
    ...
    <div class="flex">
        <flux:input icon="magnifying-glass" class="h-10 w-full max-w-sm" wire:model.live.debounce.300ms="search" placeholder="Cari nama atau email..." />
    </div>
    ...
    <flux:modal wire:model="showModal" class="max-w-md md:max-w-lg p-0 overflow-hidden bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl">
        <div class="p-6">
            ...
            <div class="max-h-[60vh] overflow-y-auto pr-2 customized-scrollbar">
                <form wire:submit="save" id="userForm" class="space-y-4">
                    <flux:field>
                    ...
                    <flux:field>
                        <flux:label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Peran (Role) <span class="text-red-500">*</span></flux:label>
                    ...
                    @if($role === 'cashier')
                        <div class="mt-4 border-t border-zinc-100 dark:border-zinc-800 pt-4">
                            <flux:label class="text-sm font-medium mb-3 block text-zinc-700 dark:text-zinc-300">Hak Akses Modul (Cashier)</flux:label>
                            <div class="grid grid-cols-2 gap-3 mt-2">
                                <flux:checkbox label="Dashboard" wire:model="permissions" value="dashboard" />
                                <flux:checkbox label="Kasir (POS)" wire:model="permissions" value="pos" />
                    ...
                    @if($editingId !== auth()->id())
                        <div class="pt-2">
                            <flux:checkbox label="Akun Aktif" wire:model="is_active" />
                        </div>
                    @endif
                </form>
            </div>
