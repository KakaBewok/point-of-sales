<div class="px-0 py-8 md:px-6 space-y-8 max-w-5xl mx-auto flex-1 w-full">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl md:text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Detail Transaksi</h1>
            <p class="text-sm text-zinc-500 mt-1">Informasi lengkap transaksi <span class="font-bold text-zinc-900 dark:text-white">#{{ $transaction->invoice_number }}</span></p>
        </div>
        <div class="flex justify-between md:justify-end w-full md:w-auto gap-3 mt-5 md:mt-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-zinc-50/50 border-b border-zinc-100 dark:bg-zinc-800/30 dark:border-zinc-800">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right font-semibold text-zinc-600 dark:text-zinc-400">Harga (Rp)</th>
                                <th class="px-6 py-3 text-right font-semibold text-zinc-600 dark:text-zinc-400">Subtotal (Rp)</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                            @foreach($transaction->items as $item)
                                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-right">{{ number_format($item->product_price, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 text-right font-semibold text-zinc-900 dark:text-white">{{ number_format($item->subtotal, 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="space-y-6">
            <div class="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <h4 class="font-semibold text-zinc-900 dark:text-white text-base mb-5">Informasi Transaksi</h4>
                <div class="space-y-5">
                    <div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $transaction->user->name ?? 'By System' }}</span>
                        </div>
                    </div>

                    <div>
                        <div class="text-[10px] text-zinc-500 uppercase tracking-widest font-black mb-1">Waktu Transaksi</div>
                        <div class="text-sm font-normal text-zinc-600 dark:text-zinc-300">
                            {{ $transaction->created_at->format('d F Y') }}
                            <span class="text-zinc-600">{{ $transaction->created_at->format('H:i:s') }}</span>
                        </div>
                    </div>
